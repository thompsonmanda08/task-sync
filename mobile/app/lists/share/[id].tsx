...
export default function ShareListScreen() {
  ...
  const userRole = list.members?.find((m) => m.userId === currentUser.id)?.role;
  ...
  const nonMemberUsers = users?.filter(
    (user) =>
      ...
      !list.members.some((member) => member.userId === user.id),
  );
  ...
  const handleShare = () => {
    if (selectedUserId) {
      shareList(list.id, selectedUserId, selectedRole);
      setSelectedUserId(null);
    }
  };

  const handleUpdateRole = (userId: string, role: Role) => {
    updateMemberRole(list.id, userId, role);
  };

  const handleRemoveMember = (userId: string) => {
    removeMember(list.id, userId);
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Share "{list.title}" with others</Text>
        ...
        <>
          ...
          {selectedUserId && (
            <View style={styles.roleSection}>
              ...
              <Button title="Share List" onPress={handleShare} fullWidth />
            </View>
          )}
        </>
      )}
