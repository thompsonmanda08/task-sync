...
func LogUserIn(c *fiber.Ctx) error {
	...
	if request.Email == "" || request.Password == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			...
			"status":  fiber.StatusBadRequest,
			"data":    fiber.Map{"error": "Email and password are required"},
		})
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"success": false,
			"message": "Failed to generate token", // FAILED TO GENERATE TOKEN
...
func RegisterNewUser(c *fiber.Ctx) error {
	...
	if request.Email == "" || request.Name == "" || request.Password == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			...
			"status":  fiber.StatusBadRequest,
			"data":    fiber.Map{"error": "Missing required fields"},
		})
	}
	...
	if err == nil {
		...
		return c.Status(fiber.StatusConflict).JSON(fiber.Map{
			...
			"data":    fiber.Map{"error": "email already registered"},
		})
		...
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			...
			"status":  fiber.StatusInternalServerError,
			"data":    fiber.Map{"error": utils.ErrString(err)},
		})
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"success": false,
			"message": "Failed to create user or default list",
			"data":    fiber.Map{"error": err.Error()},
			"status":  fiber.StatusInternalServerError,
		})
	}
